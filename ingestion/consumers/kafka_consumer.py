"""
Kafka Consumer for AIS Stream Pipeline (kafka_consumer.py)
...
consumer = KafkaConsumer(
        ...
        # Manual commit: consumer.commit() is called inside flush_buffers()
        # only after a successful sink write, guaranteeing offsets are never
        # acknowledged before the data behind them is durably persisted.
...
def flush_buffers():
    ...
    try:
        ...
        return  # don't commit if flush failed

    # Commit only after a successful flush to sink
    consumer.commit()
...
def handle_message(extracted_data: dict, message_type: str):
    ...
    if message_type in ("PositionReport", "StandardClassBPositionReport"):
        position_buffer.append({
            ...
            "position_accuracy": extracted_data.get("position_accuracy"),
            "raim": extracted_data.get("raim"),
